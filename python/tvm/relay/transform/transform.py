...
def Conv2dDenseToConv2dSparse(weight_name, weight_shape):
    """
    Rewrite qualified ```nn.conv2d operation``` to ```nn.conv2d_sparse```
    This pass is used in ```data_dep_optimization.csr_conv2d```
    Parameters of this pass is generated by ```analysis.sparse_dense.process_params```

    Parameters
    ----------
    weight_name: Array[String]
      Names of weights which qualified sparse contains

    weight_shape: Array[Array[IntImm]]
      Weights shape in CSR format.

    Returns
    -------
    ret : tvm.transform.Pass
        The registered Conv2dToSparse pass.
    """
    x = _ffi_api.Conv2dToSparse(weight_name, weight_shape)
    return x
