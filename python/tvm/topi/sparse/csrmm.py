...
def batch_csrmm(data, indices, indptr, dense, oshape):
    ...
    NNZ = simplify(indptr.shape[0]-1)
    batches, _, M = dense.shape
            ...
            if use_gpu:
                with irb.if_scope(row < NNZ):
                    with irb.for_range(0, M, for_type="vectorize", name='m') as m:
                        ...
                        with irb.for_range(0, row_elems, name='idx') as idx:
                            elem = row_start+idx
                            dot[0] += data_ptr[elem] * dense_ptr[indices_ptr[elem]*M+m]
                        out_ptr[(batch*M*NNZ) + row*M+m] += dot[0]
            else:
                with irb.for_range(0, NNZ, for_type="parallel", name='row') as row:
                    with irb.for_range(0, M, for_type="vectorize", name='m') as m:
                        dot = irb.allocate('float32', (1,), name='dot', scope='local')
                        out_ptr[(batch*M*NNZ) + (row*M+m)] = 0.
                        dot[0] = 0.
